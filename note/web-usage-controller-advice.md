...
* [springboot-web 常用异常信息返回](#springboot-web-常用异常信息返回)
  * [一. 简述](#一-简述)
  * [二. 开发](#二-开发)
    * [1. `mvn` 新增 `web-starter` 的依赖](#1-`mvn`-新增-`web-starter`-的依赖)
    * [2. 准备一个标准输出类和一些异常](#2-准备一个标准输出类和一些异常)
    * [3. 资源和资源控制器](#3-资源和资源控制器)
    * [4. 重点：使用 `ControllerAdvice` 监听控制器调用时出现的异常](#4-重点：使用-`controlleradvice`-监听控制器调用时出现的异常)
    * [5. 接口测试](#5-接口测试)
  * [三. 总结](#三-总结)
